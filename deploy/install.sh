#!/usr/bin/env bash
#
# CoolFreakinGames installer — provisions the app behind nginx on ports 80 and 443.
#
# Usage (run from the repo root, as root or via sudo):
#   sudo ./deploy/install.sh --domain coolfreakingames.dev [options]
#
# Options:
...
#   --port N           Port the Deno app listens on (default: 8001)
#   --app-dir PATH     Where the app lives (default: /opt/coolfreakingames)
...
if [[ "$USE_SERVICE" -eq 1 ]]; then
  ...
  id -u coolfreakingames >/dev/null 2>&1 || useradd --system --no-create-home --shell /usr/sbin/nologin coolfreakingames
  chown -R coolfreakingames:coolfreakingames "$APP_DIR"
  ...
  runuser -u coolfreakingames -- env DENO_DIR="$APP_DIR/.deno_cache" \
  ...
  sed -e "s|__APP_DIR__|$APP_DIR|g" \
      ...
      -e "s|Environment=PORT=8001|Environment=PORT=$APP_PORT|" \
      "$SRC_DIR/deploy/coolfreakingames.service" > /etc/systemd/system/coolfreakingames.service
  systemctl daemon-reload
  systemctl enable --now coolfreakingames
  systemctl is-active --quiet coolfreakingames && log "coolfreakingames service running" || die "service failed to start (see: journalctl -u coolfreakingames)"
else
  warn "--no-service set; ensure the app is running on 127.0.0.1:$APP_PORT"
fi

# ── TLS certificate ──────────────────────────────────────────────────────────
CERT=/etc/nginx/ssl/coolfreakingames.crt
CERT_KEY=/etc/nginx/ssl/coolfreakingames.key
...
case "$TLS_MODE" in
  ...
  letsencrypt)
    ...
    sed -e "s|__DOMAIN__|$DOMAIN|g" \
        ...
        "$SRC_DIR/deploy/nginx.conf" > /etc/nginx/sites-available/coolfreakingames.conf
    ln -sf /etc/nginx/sites-available/coolfreakingames.conf /etc/nginx/sites-enabled/coolfreakingames.conf
...
sed -e "s|__DOMAIN__|$DOMAIN|g" \
    -e "s|__CERT__|$CERT|g" \
    -e "s|__CERT_KEY__|$CERT_KEY|g" \
    -e "s|127.0.0.1:8001|127.0.0.1:$APP_PORT|g" \
    "$SRC_DIR/deploy/nginx.conf" > /etc/nginx/sites-available/coolfreakingames.conf
...
if [[ "$TLS_MODE" == "none" ]]; then
  ...
  ' "$SRC_DIR/deploy/nginx.conf" | sed -e "s|__DOMAIN__|$DOMAIN|g" \
    > /etc/nginx/sites-available/coolfreakingames.conf
fi
...
log "done."
echo
echo "  App:    127.0.0.1:$APP_PORT $( [[ $USE_SERVICE -eq 1 ]] && echo '(systemd: coolfreakingames.service)' )"
if [[ "$TLS_MODE" == "none" ]]; then
